...
class ExcelTableModel(QAbstractTableModel):
    def __init__(self, filepath, sheet_name):
        ...
        self.row_heights = {}
        self.hidden_cols = []
        self.highlighted_cells = {}  # Warna mapping permanen
        self.search_highlights = {}     # Warna hasil search (Teal)
        self.font_size = 10
        self._load_excel()
    ...
    def _load_excel(self):
        ...
        # Deteksi kolom tersembunyi dengan cara yang lebih singkat
        self.hidden_cols = [
            column_index_from_string(col_letter) - 1 
            for col_letter, col_dim in ws.column_dimensions.items() if col_dim.hidden
        ]
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.BackgroundRole:
            # 1. Prioritas: Search Highlight (Warna Teal dari Mockup)
            if (r, c) in self.search_highlights:
                return QBrush(self.search_highlights[(r, c)])

            # 2. Prioritas: Mapping Highlight
    ...
    def setData(self, index, value, role=Qt.EditRole):
        if role == Qt.BackgroundRole and index.isValid():
            ...
            if value is None or value == QColor("white"):
                if (r, c) in self.search_highlights: del self.search_highlights[(r, c)]
            else:
